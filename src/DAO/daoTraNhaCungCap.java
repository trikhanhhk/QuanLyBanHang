...
public class daoTraNhaCungCap {
    ...
    //Thêm phiếu trả mới
    public boolean InsertPhieuTra(int id_kho, String maNV) {
        ...
        DAO.daoThongBao.getInstance().insertThongBao("[Trả hàng] Nhân viên " + nv.getTenNV() + " đã trả hàng vào lúc " + ngay, ngay, 2);
        return true;
    }
    ...
    public ArrayList<PhieuTraKho> FindListTraKho(ArrayList<PhieuTraKho> DuLieuMau, String ValToSearch) {
        ...
        for (int i = 0; i < DuLieuMau.size(); i++) {
            ...
            String tennv = DAO.daoTaiKhoan.getInstance().getNhanVien(DuLieuMau.get(i).maNV).getTenNV();
